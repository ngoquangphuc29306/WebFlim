...
export default function DiscoveryFilterDialog({
  ...
  const [rawYearFrom, setRawYearFrom] = useState(currentFilters.yearFrom ? String(currentFilters.yearFrom) : '');
  const [rawYearTo, setRawYearTo] = useState(currentFilters.yearTo ? String(currentFilters.yearTo) : '');
  ...
  const activeRange = Boolean(rawYearFrom || rawYearTo || draft.yearFrom || draft.yearTo);

  const handleYearFromChange = (value: string) => {
    const cleaned = value.replace(/\D/g, '').slice(0, 4);
    setRawYearFrom(cleaned);
    const numericValue = cleaned.length === 4 ? Number(cleaned) : undefined;
    setDraft((previous) => ({ ...previous, year: undefined, yearFrom: numericValue }));
  };

  const handleYearToChange = (value: string) => {
    const cleaned = value.replace(/\D/g, '').slice(0, 4);
    setRawYearTo(cleaned);
    const numericValue = cleaned.length === 4 ? Number(cleaned) : undefined;
    setDraft((previous) => ({ ...previous, year: undefined, yearTo: numericValue }));
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-200 p-0 sm:p-4" role="dialog" aria-modal="true" aria-labelledby="filter-dialog-title">
      ...
      <div ref={dialogRef} className="relative w-full h-full sm:h-auto sm:max-w-3xl sm:max-h-[90vh] bg-[#121212] border-0 sm:border border-[#262626] sm:rounded-2xl shadow-2xl flex flex-col overflow-hidden z-10 animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between px-4 sm:px-6 py-3.5 sm:py-4 border-b border-[#262626] bg-[#161616] shrink-0">
          <div className="flex items-center gap-2">
            <Filter className="w-5 h-5 text-[#e50914]" />
            <h2 id="filter-dialog-title" className="text-base sm:text-lg font-bold text-white tracking-tight">Bộ Lọc Phim</h2>
          ...
          <button
            ...
            className="p-2 min-w-[40px] min-h-[40px] flex items-center justify-center text-[#a3a3a3] hover:text-white rounded-xl hover:bg-[#262626] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#e50914]"
          >
            <X className="w-5 h-5" />
          </button>
        </div>

        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 divide-y divide-[#262626]">
          <FilterSection title="1. Thể Loại Phim">
            <div className="flex flex-wrap gap-2">
              {genres.map((genre) => (
                <SelectionButton
                  key={genre.slug}
                  selected={draft.genre === genre.slug}
                  onClick={() => setDraft((previous) => ({ ...previous, genre: previous.genre === genre.slug ? undefined : genre.slug }))}
                >
                  {genre.name}
                </SelectionButton>
              ))}
          ...
          <FilterSection title={`2. Quốc Gia (${countries.length})`} className="pt-6">
            ...
            {!countrySearch.trim() && popularCountries.length > 0 && (
              <div className="space-y-1.5">
                <div className="text-[11px] font-semibold text-[#737373] uppercase tracking-wider">Quốc gia phổ biến:</div>
                <div className="flex flex-wrap gap-2">
                  {popularCountries.map((country) => (
                    <SelectionButton
                      key={`popular-${country.slug}`}
                      selected={draft.country === country.slug}
                      onClick={() => setDraft((previous) => ({ ...previous, country: previous.country === country.slug ? undefined : country.slug }))}
                    >
                      {country.name}
                    </SelectionButton>
                  ))}
          ...
          <FilterSection title={`3. Năm Sản Xuất (${years.length})`} className="pt-6">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2 max-h-40 overflow-y-auto pr-1">
              {filteredYears.map((year) => (
                <SelectionButton
                  key={year.slug || year.year}
                  selected={draft.year === year.year}
                  onClick={() => {
                    setRawYearFrom('');
                    setRawYearTo('');
                    setDraft((previous) => ({
                      ...previous,
                      year: previous.year === year.year ? undefined : year.year,
                      yearFrom: undefined,
                      yearTo: undefined,
                    }));
                  }}
            ...
            <fieldset disabled={!capabilities.yearRange} className="space-y-2.5 disabled:opacity-50 pt-2">
              <legend className="text-xs font-semibold text-[#a3a3a3]">Hoặc lọc theo khoảng năm</legend>
              <div className="flex flex-wrap items-center gap-2">
                <input
                  ...
                  placeholder="Từ năm"
                  aria-label="Từ năm"
                  className="w-28 h-9 bg-[#1a1a1a] text-white text-xs px-3 rounded-lg border border-[#262626] focus:outline-none focus:border-[#e50914] focus:ring-1 focus:ring-[#e50914]"
                />
                <span className="text-[#737373] text-xs font-medium">đến</span>
                <input
                  value={rawYearTo}
                ...
                {activeRange && (
                  <button
                    ...
                    onClick={() => {
                      setRawYearFrom('');
                      setRawYearTo('');
                      setDraft((previous) => ({ ...previous, yearFrom: undefined, yearTo: undefined }));
                    }}
                    className="text-xs text-[#e50914] hover:underline font-medium min-h-[36px] flex items-center px-1"
                  >
                    Xóa khoảng năm
                  </button>
                )}
          ...
          <FilterSection title="4. Loại Phim" className="pt-6">
            <div className="flex flex-wrap gap-2">
              {TYPE_OPTIONS.map((option) => (
                <SelectionButton
                  key={option.value}
                  selected={draft.type === option.value}
                  disabled={!capabilities.browseTypes.includes(option.value)}
          ...
          <FilterSection title="5. Ngôn Ngữ" className="pt-6">
            <div className="flex flex-wrap gap-2">
          ...
          <FilterSection title="6. Sắp Xếp" className="pt-6 pb-4">
            <div className="flex flex-wrap gap-2">
        ...
        {!isSupportedCombination && (
          <div className="px-4 sm:px-6 py-2.5 bg-amber-950/60 border-t border-amber-800/50 flex items-center gap-2 text-amber-200 text-xs shrink-0">
        ...
        <div
          ...
          className="flex items-center justify-between px-4 sm:px-6 py-3.5 sm:py-4 border-t border-[#262626] bg-[#161616] shrink-0 sticky bottom-0 z-20"
        >
          <button
            type="button"
            onClick={() => {
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold text-[#a3a3a3] hover:text-white hover:bg-[#262626] disabled:opacity-30 disabled:hover:bg-transparent transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#e50914] min-h-[40px]"
          >
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={handleClose}
              className="px-4 py-2.5 rounded-xl text-xs font-semibold text-[#a3a3a3] hover:text-white hover:bg-[#262626] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#e50914] min-h-[40px]"
            >
              Hủy
            </button>
            <button
              ...
              onClick={() => {
                ...
                const finalYearFrom = rawYearFrom.length === 4 ? Number(rawYearFrom) : undefined;
                const finalYearTo = rawYearTo.length === 4 ? Number(rawYearTo) : undefined;
                onApply({
                  ...draft,
              ...
              className="px-5 py-2.5 rounded-xl text-xs font-bold text-white bg-[#e50914] hover:bg-[#b80710] disabled:opacity-40 disabled:hover:bg-[#e50914] shadow-lg shadow-[#e50914]/30 transition-all flex items-center gap-1.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white min-h-[40px]"
            >
...
function FilterSection({ title, className = '', children }: { title: string; className?: string; children: React.ReactNode }) {
  return (
    <section className={`space-y-3 ${className}`}>
      <h3 className="text-xs font-bold uppercase tracking-wider text-[#d4d4d4]">{title}</h3>
      {children}
    </section>
  );
}
...
function SelectionButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`px-3 py-1.5 min-h-[34px] rounded-lg text-xs font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#e50914] disabled:cursor-not-allowed disabled:opacity-30 ${
        selected
          ? 'bg-[#e50914] text-white shadow-md shadow-[#e50914]/25 font-semibold'
          : 'bg-[#181818] text-[#a3a3a3] hover:text-white hover:bg-[#242424] border border-[#262626]'
      } ${className}`}
    >
      {children}
    </button>
  ...
  return (
    <div className="relative max-w-full sm:max-w-xs">
      <input
        ...
        aria-label={label}
        className="w-full h-9 bg-[#1a1a1a] text-white text-xs pl-8 pr-3 rounded-lg border border-[#262626] focus:outline-none focus:border-[#e50914] focus:ring-1 focus:ring-[#e50914]"
      />
